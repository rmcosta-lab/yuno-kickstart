...
- Return complete bounded artifact histories in deterministic `(created_at, id, kind)` order. Operation reads include the active post-contact escalation and notifications. Audit reads include events, business-ranked quote comparison, commitment/evidence history, recaps, briefs, recoveries, escalations, and notifications with a stable opaque cursor and limit of 1–100. `quote_comparison` preserves the accepted ranking semantics—selected/better eligible quotes first and rejected quotes afterward—and is not reordered as timeline data.
...
- Operation and audit projections reload all artifacts from PostgreSQL, order artifact histories deterministically, preserve the accepted business ranking of `quote_comparison`, enforce the 1–100 page bound, and produce stable cursors.

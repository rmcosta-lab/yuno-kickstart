...
- Bidirectional audio chunk mapping, typed tool-call parsing, `function_call_output` using the original `call_id`, the subsequent `response.create`, response audio deltas, lifecycle events, and `input_audio_buffer.speech_started` evidence with event ID, item ID, and `audio_start_ms`.
...
- `RealtimeSpeechStarted` carries event ID, item ID, and non-negative `audio_start_ms`. Other mapped values cover session readiness, speech stop, output-audio delta, response completion/cancellation, and safe provider errors. Known non-application events may be ignored; malformed mapped events fail closed with a typed error.

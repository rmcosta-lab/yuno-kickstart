...
- Every response carries `X-Request-ID`; identifiers are UUIDs; timestamps are timezone-aware UTC RFC 3339 values; dates use ISO 8601; monetary values use non-negative integer minor units and an uppercase ISO currency code. P0 accepts `MXN` only.
...
| --- | --- | --- | --- | --- |
| `POST /v1/operation-drafts` | `create_operation_draft` | `CreateOperationDraftRequest` | `201 OperationDraftResponse` | Retain source prompt and policy version; no authority is created. |
...
| `GET /v1/operations/{operation_id}` | `get_operation` | Path UUID | `200 OperationResponse` | Return the current aggregate without raw provider data. |
...
| `GET /v1/operations/{operation_id}/audit` | `get_operation_audit` | Cursor and limit query | `200 AuditTimelineResponse` | Return ordered safe events, comparison rows, and opaque next cursor. |
...
- Intake: source prompt, requested language, extraction policy version, proposed route and pickup date, proposed mandate, validation issues, approval eligibility, draft version, and timestamps.
...
- Operation: route, synthetic cargo label, status, operation version, active mandate, negotiation summary, active commitment, open escalation, and notifications.
...
- Recovery, escalation, and notification: fixed scenario, before/after operation versions, decision reason, attempted alternatives, recommended action, resolution state, acknowledgement state, and correlation ID.
- Audit: append-only event ID, operation version, safe actor kind, event type, timestamp, correlation ID, safe metadata, quote comparison rows, and opaque pagination cursor.

## Safe error and authorization semantics

`ApiErrorResponse` contains only `code`, `message`, `request_id`, optional field issues, optional resource ID, and optional current operation version. It never includes submitted secrets, raw prompts, authorization headers, stack traces, exception names, provider responses, or full request bodies.
...
- `make generate` deterministically updates `api/openapi.json` and `frontend/src/lib/api/generated/**`; a second generation produces no diff.
...
| `backend/**`, non-generated frontend source | No Fase 04 writer | Explicitly out of scope. |

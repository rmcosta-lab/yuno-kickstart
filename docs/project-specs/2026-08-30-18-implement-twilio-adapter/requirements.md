...
- `yuno_backend.volta.telephony.repositories.OutboundCallAttemptRepository`: lock/read/record operations keyed by the logical idempotency key and fingerprint so replay and uncertain results survive process failure.
...
- `yuno_backend.integrations.twilio.outbound.TwilioOutboundCallGateway`: constructed with injected HTTP client, redacted config, allowlist resolver, attempt repository, and clock; owns URLs, Basic authentication, form mapping, response/status parsing, and transport-error translation.

...
Read `AGENTS.md` and every existing target file completely. Inspect the current branch, worktree, remote default branch, open phase/specification pull requests, and diffs that already touch the requested files. Preserve unrelated changes.
...
- Include a small, directly required specification change in the active phase pull request.
- Use a short-lived branch such as `docs/specs-{topic}` for initial specs, broad reorganization, or a change unrelated to one phase.
...
Do not infer a product pivot, new infrastructure, production access, or financial operation. A developer preference is not enough to change a shared architecture or data/security boundary; record the team decision.

When a change affects an active phase, record the impact in that phase's `plan.md` or pull-request body and tell affected owners to refresh their branch. Pause only the integration work that depends on the unsettled decision.
...
Future unstarted phases may be added, removed, or reorganized. Do not silently rename, remove, or weaken the gate of an active or merged phase. Use an explicit clarification for a small correction and add a follow-up phase when the desired outcome or prerequisites materially change.
...
Review the complete diff, run `writing-guidelines` on changed prose, validate the roadmap graph, and check for secrets, personal data, dynamic phase status, and unsupported provider claims.
...
When the change belongs to a phase, include the same decision and impact in that phase's pull-request body instead of opening a second pull request.
